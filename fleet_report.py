...
def car_wear(car: dict) -> float:
    ...
    Returns 0.0 for cars with no last_service_km reading so the report
    does not crash when a new car has no service history yet.
    """
...
def print_report(fleet: list) -> None:
    """Print the nightly fleet report and append a timestamped entry to the log file."""
